...
async function talkWith(conversation) {
    try {
        ...
        const response = await openai.createChatCompletion({
            model: "gpt-3.5-turbo",
            messages: [{ role: "system", content: 'You are a tech assistant, your work is only to answer the question very briefly to a certain limit and then recommend students a particular field that you guess is better for them '},{ role: "user", content: conversation }],
        });
        ...
        if (error.code === 'ENOTFOUND' && error.hostname === 'api.openai.com') {
            ...
            throw new Error('Unable to connect to the OpenAI API. Please check your internet connection.');
