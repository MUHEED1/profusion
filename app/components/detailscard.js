...
const Detailscard = ({ src, lable, Heading, content }) => {
  ...
  return (
    ...
    onMouseEnter={()=>setisin(true)} onMouseLeave={()=>setisin(false)} className={`p-[0.7rem]   ${path=="/project"?"bg-[#f1f1f1]":"bg-[#282828] text-white"}  rounded-2xl shadow-xl`}>
      <div className="w-[full] h-[230px]   mb-5">
        <Image src={src} className=" object-cover rounded-xl h-[230px]" />
      </div>
      <div className={`text-[0.85rem] flex justify-center rounded-[999px] font-Matter  p-[0.7rem] ${path=="/project" ? "text-black bg-[#E1E1E1]":"bg-black text-white"}  w-max `}>
        {lable}
      </div>
      <div className={` text-[1.3rem] py-[0.5rem] ${path=="/project" ? "text-black":" text-white"} font-Matter font-semibold`} >
        {" "}
        {Heading}
      </div>
      ...
      <div className=" flex justify-between items-center">
        <div className={`${path=="/project" ? "text-[#333333]":" text-white"} font-Matter text-[0.89rem]`}>
          19.5K People Viewed
        </div>
